...
pub fn parse_test_line_status_pass() {
    ...
    assert_that!(&event, structure!(TestRunEvent::TestFinished [
        eq(SingleTest::new("add_2_and_4_is_6".to_string(), SingleTestStatus::Passed))
    ]));
}
...
pub fn parse_test_line_status_fail() {
    ...
    assert_that!(&event, structure!(TestRunEvent::TestFinished [
        eq(SingleTest::new("when_grcov_is_not_installed_error_is_reported".to_string(), SingleTestStatus::Failed))
    ]));
}
